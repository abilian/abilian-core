...
from abilian.web.views import ObjectEdit, ObjectDelete
...
def schedule_entities_reindex(entities):
  ...
  return index_update.apply_async(kwargs=dict(index='default', items=entities))
...
class NSView(View):
  ...
  def do_merge(self):
    ...
    already_tagged = sa.sql.select([tbl.c.entity_id]).where(tbl.c.tag_id == target.id)
    del_dup = tbl.delete()\
...
class TagPanel(AdminPanel):
  ...
  def install_additional_rules(self, add_url_rule):
    ...
    ns_base = '/<string:ns>/'
    add_url_rule(ns_base, endpoint='ns',
                 view_func=NSView.as_view('ns', view_endpoint=panel_endpoint))

    tag_base = ns_base + '<int:object_id>/'
    add_url_rule(tag_base, endpoint='tag_edit',
                 view_func=TagEdit.as_view('tag_edit', view_endpoint=panel_endpoint))

    add_url_rule(tag_base + 'delete', endpoint='tag_delete',
                 view_func=TagEdit.as_view('tag_delete',
