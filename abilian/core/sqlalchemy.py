    ...
    class SQLAlchemy(AbilianBaseSAExtension):
        ...
        def create_scoped_session(self, options=None):
            ...
            return sa.orm.scoped_session(
                partial(SignallingSession, self, **options),
                scopefunc=scopefunc)
...
def filter_cols(model, *filtered_columns):
    ...
    return list(set(p.key for p in m.iterate_properties if hasattr(
        p, 'columns')).difference(filtered_columns))
